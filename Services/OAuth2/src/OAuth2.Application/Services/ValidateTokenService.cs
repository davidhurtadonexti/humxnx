...
{
    public class ValidateTokenService : IValidateToken
    {
        IConnection _connection;
        public ValidateTokenService(IConnection connection)
        {
            _connection = connection;
        }
        public async Task<TokenDataReturn> GetDataToken(string authorizationId)
        {
            return await _connection.GetOne<TokenDataReturn>($@"sp_get_token_data '{authorizationId}'");
        }
        public async Task<Resultado> UnauthorizeToken(Guid id)
        {
            ...
            return await _connection.UpdateDataById("sp_unauthorize_token", parametros);
        }
    }
}
