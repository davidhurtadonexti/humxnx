using System;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;

namespace Auth.Infrastructure.DataBaseContext.Migrations
{
    [DbContext(typeof(AuthDbContext))]
    [Migration("CustomMigration_CrudResource")]
    public class CrudResource : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                @"
				/*************************************************************************                
				 Nombre    : CrudResource
				 Creado por   : Ronald Javier Rivera Carranza                
				 Fecha de creación : 06-Sep-2023           
				 Ej de ejecución  :                
                
				 exec CrudResource 3, 'CB1925E6-B71B-4A0A-A13B-E80D0E1E2871', '', ''
                
				 Crud resource, ingreso, actualizacion y eliminar     
				  ------------------- ---------------------------------------------------------------------- ------------------    
				 FECHA MODIFICACION						MOTIVO													  USUARIO    
				 ------------------- ----------------------------------------------------------------------- ------------------    	
					 06-Sep-2023             Se crea el proceso para crud                                       Ronald Rivera
					 06-Sep-2023             Eliminar fechas                                                    Ronald Rivera
				**************************************************************************/
				CREATE PROCEDURE dbo.CrudResource(
					@Accion int,
					@id uniqueidentifier,
					@menu_id uniqueidentifier = NULL,
					@resource nvarchar(50) = NULL
				)
				AS
				BEGIN
					--insert
					IF @Accion = 1
						BEGIN
							INSERT INTO Resources 
							(id, 
							menu_id, 
							resource,
							created_at, 
							updated_at, 
							deleted_at)
							VALUES(@id, 
							@menu_id, 
							@resource, 
							getdate(), 
							null, 
							null);
							SELECT * FROM Resources WHERE id=@id;
						END
					--update
					IF @Accion = 2
						BEGIN
							UPDATE Resources 
							SET id=@id, 
							menu_id=@menu_id, 
							resource=@resource, 
							updated_at=getdate()
							WHERE id=@id;
							SELECT * FROM Resources WHERE id=@id;
						END
					--delete logic
					IF @Accion = 3
						BEGIN
							UPDATE Resources 
							SET 
							updated_at=getdate(), 
							deleted_at=getdate()
							WHERE id=@id;
							SELECT * FROM Resources WHERE id=@id;
						END
				END;
                ");
        }
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("DROP PROCEDURE dbo.CrudResource");
        }
    }
}

